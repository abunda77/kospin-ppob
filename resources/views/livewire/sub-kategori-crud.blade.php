<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100">
            {{ __('Manajemen Sub Kategori') }}
        </h2>
        <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            <div class="relative w-full sm:w-48">
                <flux:select wire:model.live="filterKategori" placeholder="Filter Kategori">
            ...
            <flux:button wire:click="create" class="!bg-gradient-to-r !from-indigo-600 !to-violet-600 hover:!from-indigo-500 hover:!to-violet-500 !text-white !border-0 shadow-lg shadow-indigo-500/20 transition-all duration-200 hover:scale-[1.02] whitespace-nowrap">
                ...
                <span>{{ __('Tambah Sub Kategori') }}</span>
            </flux:button>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                ...
                <tbody class="bg-white divide-y divide-zinc-200 dark:bg-zinc-900 dark:divide-zinc-700">
                    @forelse ($subKategoris as $subKategori)
                        <tr wire:key="subkategori-{{ $subKategori->id }}" class="group hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-all duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $kodeUpper = strtoupper($subKategori->kategori->kode ?? '');
                                ...
                                @endphp
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset {{ $badgeColorData }}">
                                    {{ $subKategori->kategori->nama ?? '-' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-700/10 dark:bg-indigo-400/10 dark:text-indigo-400 dark:ring-indigo-400/30">
                                    {{ $subKategori->produk_ppob_count }} Produk
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($subKategori->aktif)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <flux:button wire:click="edit({{ $subKategori->id }})" size="sm" icon="pencil-square" variant="ghost" class="!px-2" title="Edit" />
                                    <flux:button wire:click="confirmDelete({{ $subKategori->id }})" size="sm" icon="trash" variant="ghost" class="!px-2 text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20" title="Delete" />
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-zinc-500 dark:text-zinc-400">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    ...
                                    <p>{{ __('Tidak ada sub kategori ditemukan.') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <flux:modal wire:model="showModal" class="md:min-w-[32rem]">
        <div class="mb-4">
            <h3 class="text-lg font-medium leading-6 text-zinc-900 dark:text-zinc-100">
                {{ $isEditing ? __('Edit Sub Kategori') : __('Tambah Sub Kategori') }}
            </h3>
            <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                {{ $isEditing ? __('Update data sub kategori.') : __('Tambah sub kategori baru.') }}
            </p>
        </div>

        <form wire:submit="save" class="space-y-6">
            <flux:select wire:model="kategoriId" label="{{ __('Kategori') }}" placeholder="Pilih Kategori">
                @foreach($kategoris as $kat)
                    <option value="{{ $kat->id }}">{{ $kat->nama }}</option>
                @endforeach
            </flux:select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <flux:input wire:model="kode" label="{{ __('Kode') }}" placeholder="SUBKAT001" />
                <flux:input wire:model="urutan" label="{{ __('Urutan') }}" type="number" min="0" placeholder="0" />
            </div>
            
            <flux:input wire:model="nama" label="{{ __('Nama Sub Kategori') }}" placeholder="Nama sub kategori" />
            
            <flux:textarea wire:model="deskripsi" label="{{ __('Deskripsi') }}" placeholder="Deskripsi sub kategori (opsional)" rows="3" />
        ...
        <div class="space-y-4">
            <div>
                ...
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-2">
                    {{ __('Apakah Anda yakin ingin menghapus sub kategori ini? Tindakan ini tidak dapat dibatalkan.') }}
                </p>
            </div>
